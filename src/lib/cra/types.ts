...
export type Product = { id: number; categoryId: CategoryId; subcategory: string | null; name: string; description: string; price: number; imageUrl: string; available: boolean; soldOut: boolean; isCustom: boolean; customKind: CustomKind | null; sortOrder: number; isFavorite: boolean };
...
export type InventoryItem = { id: number; name: string; unit: InventoryUnit; quantity: number; lowThreshold: number; available: boolean };
export type RecipeLine = { id: number; productId: number; inventoryItemId: number; quantity: number };
export type InventorySnapshot = { items: InventoryItem[]; recipes: RecipeLine[] };
