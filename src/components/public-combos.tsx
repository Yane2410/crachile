...
import type { Catalog, Combo, Product } from "@/lib/cra/types";
...
function ComboPhoto({ combo, eager = false }: { combo: Combo; eager?: boolean }) {
  ...
  return <img src={combo.imageUrl} alt={combo.name} loading={eager ? "eager" : "lazy"} decoding="async" className="size-full object-cover" />;
}
...
export function PublicCombos({ catalog }: { catalog: Catalog }) {
  const combos = (catalog.combos ?? []).filter(combo => combo.available);
  const products = catalog.products ?? [];
  const [building, setBuilding] = useState<Combo | null>(null);
  if (!combos.length) return null;
  const carousel = combos.length > 1;
  return <><section id="combos" className="-mx-0 border-b border-border/80 bg-surface-2 py-6"><div className="w-full"><div className="mb-3 flex items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-kicker text-heart">Especiales CRA</p><h2 className="mt-1 font-display text-2xl font-semibold text-title">Combos</h2><p className="mt-1 text-sm text-muted">Más por menos, sin complicar tu pedido</p></div><Gift className="size-6 shrink-0 text-heart" /></div><div className="relative">{carousel && <button type="button" aria-label="Ver combos anteriores" onClick={() => document.getElementById("cra-combos-carousel")?.scrollBy({ left: -300, behavior: "smooth" })} className="absolute left-1 top-1/2 z-10 hidden -translate-y-1/2 rounded-full bg-surface p-1.5 shadow-md lg:inline-flex"><ChevronLeft className="size-4" /></button>}<div id="cra-combos-carousel" className="flex snap-x snap-mandatory gap-3 overflow-x-auto scroll-smooth pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{combos.map((combo, index) => <article key={combo.id} className="flex h-[176px] w-[min(88vw,430px)] shrink-0 snap-start flex-row-reverse overflow-hidden rounded-[var(--radius-lg)] bg-surface shadow-[var(--shadow-border)] sm:h-[184px] sm:w-[430px]"><div className="w-[40%] shrink-0 overflow-hidden bg-surface-2"><ComboPhoto combo={combo} eager={index < 2} /></div><div className="flex min-w-0 flex-1 flex-col justify-between p-3 sm:p-4"><div><div className="flex items-start gap-2"><h3 className="min-w-0 flex-1 line-clamp-2 font-display text-lg font-semibold leading-tight text-title">{combo.name}</h3><span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-heart/10 px-2 py-1 text-[10px] font-semibold leading-tight text-heart"><Tag className="size-3" /> {benefitLabel(combo)}</span></div><p className="mt-2 line-clamp-3 text-xs leading-relaxed text-muted sm:text-sm">{combo.description}</p></div><button type="button" onClick={() => setBuilding(combo)} className="mt-2 w-full rounded-xl bg-heart px-3 py-2 text-sm font-bold text-white">Armar combo</button></div></article>)}</div>{carousel && <button type="button" aria-label="Ver más combos" onClick={() => document.getElementById("cra-combos-carousel")?.scrollBy({ left: 440, behavior: "smooth" })} className="absolute right-1 top-1/2 z-10 hidden -translate-y-1/2 rounded-full bg-surface p-1.5 shadow-md lg:inline-flex"><ChevronRight className="size-4" /></button>}</div></div></section>{building && <ComboBuilder combo={building} products={products} onClose={() => setBuilding(null)} />}</>;
}
